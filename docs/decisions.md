...
## D-030 — Tareas genéricas del proyecto, exclusivas de administración y supervisión

**Estado:** aceptada (retoma la puerta que dejó abierta D-022: tareas genéricas "por fuera de las etapas pero siempre atadas a un proyecto").

**Contexto:** la pestaña Tareas del detalle de proyecto mostraba la tabla legada "Por hacer / Hechas" (`tareasIniciales`, tipo `Tarea`), desconectada del seguimiento real. La operación necesita tareas del proyecto que no pertenezcan a fabricación ni instalación (trámites, permisos, coordinaciones) con la misma interfaz que el resto del seguimiento.

**Decisión:** `GrupoTareaPresupuesto` suma el grupo **`generales`** con el pseudo-tipo `TIPO_TAREA_GENERAL` ("general", fuera del catálogo; `null` en PostgreSQL). La pestaña **Tareas** del detalle pasa a mostrar `TareasGenerales` (`components/proyectos/TareasGenerales.tsx`): misma tabla estructurada, misma `FilaTarea` compartida (ahora exportada), mismos diálogos de completar/editar/archivar y misma auditoría que Fábrica/Instalación; el bloque se etiqueta "Proyecto · General". El alta pide nombre, prioridad, responsable y fechas (sin componente del presupuesto ni etapa). **Acceso —vista y edición— solo para administración y supervisión** (permiso `verTareasGeneralesProyecto`): el rol Usuario no ve la pestaña, y las tareas generales se excluyen de su lista global incluso si figuran asignadas a él (filtro en `Todo.tsx` y en las lecturas `/api/v2`). La tabla legada "Por hacer / Hechas" se retira del detalle. Migración `018_tareas_generales_proyecto.sql`: `tipo_producto` pasa a nullable, el `check` de grupo admite `generales` y exige `tipo_producto is null` exactamente para ese grupo; `POST /proyectos/:id/tareas` acepta el grupo sin producto.

**Consecuencias:** las genéricas cuentan en el avance general del proyecto (tareas completadas/total) pero no en los porcentajes de Fábrica/Instalación, que filtran por grupo. En la sección Tareas global aparecen con bloque "Proyecto · General" para administración y supervisión. `tareasIniciales`/`Tarea` quedan usadas solo por el Dashboard, pendientes de retirar.

## D-031 — Evidencia multi-formato, justificaciones con mínimo de 50 caracteres y filtros en las matrices por proyecto

**Estado:** aceptada.

**Contexto:** la evidencia de cierre solo admitía imágenes, cuando muchas tareas (sobre todo las genéricas del proyecto: trámites, permisos) se respaldan con PDFs, planillas o enlaces a documentos. Las justificaciones de texto (motivos de reapertura, pausa, cancelación) aceptaban cualquier contenido, incluso un punto. Y las matrices de tareas dentro del proyecto no tenían el filtro por columna que ya existía en la sección Tareas.

**Decisión:** (1) **Evidencia multi-formato, exclusiva de las tareas generales del proyecto:** solo para el grupo `generales`, el diálogo de completar acepta imágenes (se comprimen, máx. 12 MB), documentos PDF/Word/Excel/CSV/texto (sin comprimir, máx. 4 MB en Fase 2) o un **enlace externo** (`esEnlace` en `EvidenciaTarea`; el campo se ignora si se adjunta archivo). Las tareas de etapas de fabricación/instalación (premarcos incluidos) y la evidencia de cierre de proyecto siguen exigiendo **fotografía** (`prepararEvidencia` con `soloImagen` rechaza cualquier otro tipo). La vista de evidencia rinde imagen, ancla al enlace o descarga del documento según el tipo. En PostgreSQL no hace falta migración: `evidencias.mime` ya es genérico y los enlaces se guardan con mime `enlace/url` y la URL en `ruta_archivo`. (2) **Justificación mínima transversal:** `lib/justificaciones.ts` define `MINIMO_JUSTIFICACION = 50`; aplican el mínimo el motivo de reapertura de tareas, los cinco motivos del tablero (pausa, reanudación, cancelación, reactivación, reapertura de proyecto) y las observaciones de cierre cuando se escriben (siguen siendo opcionales). Cada campo muestra cuántos caracteres faltan; `/api/v2` replica la validación en reabrir y completar. (3) **Filtros por columna en las matrices del proyecto:** se extrae `TablaTareasSeguimiento` (tabla compartida con `useTablaFiltrable` + `EncabezadoFiltrable`) y la usan todos los bloques de Fábrica/Instalación y las tareas genéricas: ordenar y filtrar por Tarea, Detalle, Responsable, Fechas y Prioridad, igual que la sección Tareas (D-019).

**Consecuencias:** el mismo componente de tabla sirve a las tres vistas (bloques por producto, generales y futuras), así los filtros y columnas no divergen. El límite de 4 MB para documentos es una restricción de `localStorage` de Fase 2; el backend con archivos reales podrá elevarlo. Las tareas genéricas aparecen en la sección Tareas global con bloque "Proyecto · General" (verificable en el filtro de la columna Bloque). Un texto de menos de 50 caracteres ya no es justificación válida en ninguna pantalla ni en la API.

## D-032 — Estado "Pendiente" del proyecto (dependencia del cliente) y tooltips explicativos con delay

**Estado:** aceptada.

**Contexto:** el tablero solo distinguía "Pausada" para todo proyecto detenido, sin diferenciar la causa. La operación necesita separar las detenciones por una **cuestión interna** de Italplast (falta de material, reprogramación de cuadrilla) de las que dependen de una **acción del cliente** (aprobación, pago, definición), porque se gestionan y se comunican distinto. Además, los estados del tablero y las etapas de producto (fabricación de premarcos, instalación de premarcos, fábrica, instalación) no explicaban su significado a quien recién se suma.

**Decisión:** (1) `EstadoObra` suma el valor **`pendiente`**, que se comporta como una **columna propia del tablero entre Planificadas y En progreso**. Pasar a Pendiente exige un motivo (qué se espera del cliente), sujeto al mínimo de 50 caracteres de [[justificaciones]] (D-031); se registra en `historialEstados` y en un nuevo array `pendientes` de `RegistroPendiente` (análogo a `pausas`/`RegistroPausa`, con `resueltaEn`/`resueltaPorId`/`motivoResolucion`). Reanudar desde Pendiente vuelve a En progreso con observación obligatoria y cierra el registro abierto. "Pausada" se redefine explícitamente como detención por **causa interna**; "Pendiente" como espera del **cliente**. El estado tiene su propio token de color (`--estado-pendiente`, violeta) usado en `EstadoBadge`, la tarjeta del tablero y el filtro de la vista Tarjetas. (2) **Tooltips explicativos con delay de 1 s** (`TooltipProvider delayDuration={1000}`, primer uso del componente en el repo): en los encabezados de columna del tablero (qué significa cada estado) y en los títulos de bloque de Fábrica/Instalación del detalle (qué significa cada etapa, vía `descripcionGrupo` en `lib/seguimiento-presupuesto.ts`). (3) Backend: migración `019_estado_pendiente_proyecto.sql` amplía el `check` de `proyectos.estado` con `'pendiente'` y crea `proyecto_pendientes` (espejo 1:1 de D-027); las rutas `/api/v2` no tenían transiciones implementadas, así que no requieren cambios.

**Consecuencias:** el flujo del tablero pasa a cinco columnas ordinarias (Planificadas, Pendientes, En progreso, Pausadas, Finalizadas). Un proyecto en Pendiente sigue siendo arrastrable y cuenta en las mismas reglas que Pausada (no puede volver a Planificada con avance, etc.). El componente `Tooltip` queda disponible para futuras ayudas contextuales. Cambiar entre Pausada y Pendiente no cierra automáticamente el registro abierto del estado anterior, igual que las demás transiciones no especiales del tablero (limitación conocida heredada).

## D-033 — Cronograma general (Gantt de capacidad) por proyecto × producto

**Estado:** aceptada.

**Contexto:** el cronograma real de fábrica de Italplast (Excel "Cronograma Fábrica") no solo planifica cada obra hacia atrás (D-023), sino que **compara la carga de todas las obras por fecha** para anticipar la saturación de fábrica e instalación. Ese cruce de capacidades es clave para todas las áreas y no existía en la app: solo había el detalle por proyecto.

**Decisión:** nueva **tercera vista de Proyectos**, "Cronograma" (Gantt general, junto a Tarjetas y Tablero), en `src/frontend/src/components/proyectos/GanttProyectos.tsx`, alimentada por `src/frontend/src/lib/cronograma.ts` sobre el cálculo backward existente. (1) **Filas por proyecto × producto operativo** (una obra multiproducto ocupa varias filas; Servicios y las obras finalizadas/canceladas se excluyen). Cada fila dibuja las ventanas de fabricación de premarcos, fábrica e instalación como barras, más los hitos **Firma de Presupuesto Ejecutivo** (diamante rojo) y **confirmación del cliente** (diamante gris). (2) **Eje temporal con zoom Día/Semana/Mes** (semana por defecto), filtros por producto/líder/cliente, y sección **"Sin compromiso"** para los productos operativos sin planificación. Solo lectura; las fechas se editan en el detalle. Visible para **todos los roles**. (3) **Carril de capacidad**: por cada línea de producto en fábrica y para la instalación, se suma la demanda diaria de aberturas (`aberturas/día = total ÷ días de la etapa`, reparto lineal) y se toma el **pico por bucket**, marcándolo en **rojo cuando supera el tope**. (4) **Topes de capacidad configurables** (`src/frontend/src/lib/capacidad.ts`, sección nueva en Reglas y catálogo): tope diario **por línea de producto** para fábrica y un **tope único** para instalación (cuadrillas); predeterminados 6 y 10 ab./día. (5) **Planificación backward obligatoria al crear el proyecto** para cada producto operativo (fecha ancla + días de instalación y de fábrica, y días de fabricación de premarcos si aplica); así todo proyecto nuevo nace con barras. (6) El hito de firma se **materializa como tarea genérica** "Firma de Presupuesto Ejecutivo" (grupo `generales`, D-030) autogenerada al crear el proyecto, con la fecha de firma **más temprana entre los productos** (un presupuesto por proyecto mezcla PVC y aluminio y comparten el hito). (7) Backend: migración `020_capacidad_produccion.sql` (`capacidad_instalacion` fila única + `capacidad_fabrica` por producto), espejo 1:1 de D-027.

**Consecuencias:** el cruce de capacidades queda disponible para todas las áreas y detecta sobrecarga por semana/mes. La obligatoriedad de planificación cambia el flujo de alta: un producto operativo ya no puede crearse sin fecha ancla ni duraciones (Servicios sigue exento). Los proyectos previos sin planificación (legado) aparecen en "Sin compromiso" hasta completarlos; los cuatro proyectos demo se enriquecieron con planificación (D-018). La demanda usa reparto lineal, no unidad por unidad; la instalación no distingue línea de producto. El componente es solo lectura: reprogramar arrastrando barras queda pendiente. El backend persiste los topes pero el frontend activo sigue en `localStorage`.
