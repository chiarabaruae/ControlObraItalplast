...
- La tabla "Tareas internas" fue retirada de la sección Tareas: las tareas dependen de una etapa o del grupo `generales` del proyecto. El tipo `Tarea` y `tareasIniciales` quedan como legado usado solo por el Dashboard, pendiente de retirar.
- **Tareas genéricas del proyecto (D-030):** la pestaña Tareas del detalle gestiona tareas del grupo `generales`, atadas directamente al proyecto (sin producto ni etapa), con la misma tabla, diálogos y auditoría que Fábrica/Instalación y bloque "Proyecto · General". Vista y edición exclusivas de administración y supervisión: el rol Usuario no ve la pestaña ni esas tareas en su lista global, aunque figuren asignadas a él. Fuente: `src/frontend/src/components/proyectos/TareasGenerales.tsx`.
...
- Completar una tarea exige una **evidencia**. Las tareas de etapas de fabricación/instalación exigen fotografía; solo las **tareas generales del proyecto** admiten además documentos (PDF, Word, Excel, CSV, texto) o un enlace externo (D-031). La observación es opcional, pero si se escribe debe tener al menos 50 caracteres.
- **Justificación mínima transversal (D-031):** todo motivo escrito (reapertura de tareas, pausa/reanudación/cancelación/reactivación/reapertura de proyectos) exige 50 caracteres como mínimo; los campos indican cuántos faltan y la API replica la validación. Fuente: `src/frontend/src/lib/justificaciones.ts`.
- Las imágenes se redimensionan a un máximo de 1280 px y se comprimen a JPEG antes de guardarse localmente; los documentos se guardan sin comprimir con un máximo de 4 MB en Fase 2; los enlaces guardan solo la URL (`esEnlace`).
- **Las matrices de tareas del proyecto (bloques de Fábrica/Instalación y tareas genéricas) tienen filtro y orden por columna** (Tarea, Detalle, Responsable, Fechas, Prioridad), igual que la sección Tareas, mediante la tabla compartida `TablaTareasSeguimiento`.
